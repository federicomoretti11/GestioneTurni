...
interface DipStat {
  dipendente_id: string; nome: string; cognome: string
  turni_confermati: number; turni_con_timbratura: number; geo_anomalie: number
  ore_pianificate: number; ore_effettive: number | null
}
interface Sommario {
...
// ── Page ─────────────────────────────────────────────────────
export default async function AnalyticsPage({ searchParams }: { searchParams: { mese?: string } }) {
  ...
  // Fetch API lato server — usa headers per preservare cookie e X-Tenant-Id
  const hdrs = headers()
  const host = hdrs.get('host') ?? 'localhost'
  const proto = host.includes('localhost') ? 'http' : 'https'
  const mese = searchParams.mese
  const url = `${proto}://${host}/api/admin/analytics${mese ? `?mese=${mese}` : ''}`
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        ...
        <MetricCard label="Ore pianificate" valore={`${sommario.totale_ore_pianificate}h`} sub="somma dipendenti" accent="teal" />
      ...
      <div className="rounded-xl bg-white border border-slate-900/20 overflow-hidden" style={{ boxShadow: '0 1px 2px rgba(15,23,42,.04)' }}>
        ...
        {ore_per_dipendente.length === 0 ? (
          <div className="px-5 py-8 text-center">
            <p className="text-sm text-slate-400">Nessun turno registrato nel periodo selezionato — i dati appariranno man mano che vengono inseriti i turni.</p>
          </div>
        ) : (
