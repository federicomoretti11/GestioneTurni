...
export default function CalendarioProgrammazionePage() {
  ...
  async function eseguiSalvataggio(payload: { template_id: string | null; ora_inizio: string; ora_fine: string; posto_id: string | null; note: string; dipendente_id?: string; dipendente_custom_id?: string }): Promise<string | void> {
    // Turno notturno: split automatico se ora_fine < ora_inizio (e non è mezzanotte esatta)
    ...
    if (crossaMezzanotte) {
      ...
      const baseBody = { ...payload, dipendente_id: payload.dipendente_id ?? modale.dipendenteId, stato: 'bozza' }
